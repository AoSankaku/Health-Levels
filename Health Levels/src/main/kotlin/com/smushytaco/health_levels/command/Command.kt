...
object Command {
    private fun getter(response: (HealthLevelsXP) -> Text, literal: String = "check"): LiteralArgumentBuilder<ServerCommandSource> {
        return LiteralArgumentBuilder.literal<ServerCommandSource>(literal)
            ...
            .then(CommandManager.argument("player", EntityArgumentType.player())
                ...
                .executes { ctx ->
                    ctx.source.sendFeedback(Text.literal("ochinchin"),true)
        ...
        return CommandManager.literal(literal)
            ...
            .then(CommandManager.argument("players", EntityArgumentType.players())
                .then(CommandManager.argument("amount", IntegerArgumentType.integer(0))
                    .executes { ctx ->
                        ctx.source.sendFeedback(Text.literal("Changing" + "a"),true)
    ...
    public fun buildHealthLevelsCommand(): LiteralArgumentBuilder<ServerCommandSource> {
        val base = CommandManager.literal("healthlevels")
        listOf(
            setter("setxpong") { target, healthXP ->
                println("1 Sucks Fabric world!")
                if (target !is HealthLevelsXP) return@setter
                println("2 Sucks Fabric world!")
                target.healthXP = healthXP
                println("3 Sucks Fabric world!")
                target.onModified()
            },
    ...
    fun buildHpxpCommand(): LiteralArgumentBuilder<ServerCommandSource> {
        val base = CommandManager.literal("hpxp")
        listOf(
            setter("set") { target, healthXP ->
                if (target !is HealthLevelsXP) return@setter
                target.healthXP = healthXP
                target.onModified()
            },
            setter("add") { target, healthXP ->
                if (target !is HealthLevelsXP) return@setter
                target.healthXP += healthXP
                target.onModified()
            },
