...
object HealthMethods {
    ...
    fun PlayerEntity.deathPenalty() {
        if (this !is HealthLevelsXP) return
        if (config.loseType != LoseType.NONE ) healthXP = 0
        if (config.loseType == LoseType.LEVELS_AND_XP) healthLevel = 0
        if (config.loseType == LoseType.HALF_LEVELS_AND_ALL_XP) healthLevel /= 2
        if (config.loseType == LoseType.SPECIFIC_LEVELS_AND_ALL_XP) healthLevel -= config.loseAmount
